...
namespace net
{
    ...
    {
        uint32_t lehmer = 0;
        uint32_t random() { lehmer += 0xe120fc15; uint64_t tmp; tmp = (uint64_t)lehmer * 0x4a39b70d; uint32_t m1 = (tmp >> 32) ^ tmp; tmp = (uint64_t)m1 * 0x12fad5c9; uint32_t m2 = (tmp >> 32) ^ tmp; return m2; }
    }
    ...
    public:
        ...
        connection(owner parent, asio::io_context &asioContext, asio::ip::tcp::socket socket, ts_queue<owned_message<T>> &qIn, float versionNumber)
    ...
    private:
        ...
        {
            uint64_t v_num = (uint64_t)(versionNumber * 1000.0f);
            uint64_t out;
            input &= input >> 1;
    ...
    {
    public:
        client_interface(float versionNumber) : m_socket(m_context)
        {
            this->versionNumber = versionNumber;
        }
        ...
        bool connect(const std::string &host, const uint16_t port)
        {
            try
            {
                ...
                m_connection = std::make_unique<connection<T>>(connection<T>::owner::client, m_context, asio::ip::tcp::socket(m_context), m_qMessagesIn, versionNumber);
    ...
    {
    public:
        server_interface(uint16_t port, float versionNumber)
            ...
            m_asioAcceptor.async_accept(
                [this](std::error_code ec, asio::ip::tcp::socket socket)
                {
                    if(!ec)
                    {
                        ...
                        std::shared_ptr<connection<T>> newconn = std::make_shared<connection<T>>(connection<T>::owner::server, m_asioContext, std::move(socket), m_qMessagesIn, versionNumber);
